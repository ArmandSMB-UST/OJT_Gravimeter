...
function computeAnomaly(){
  for (var i = Math.floor(windowHeight/1.50); i < windowHeight; i+=10){
    for (var j = 0; j < windowWidth; j+=10){
        ...
        anomaliesArray[j] = summation * G / 1000**2;
    }
  }
}
...
function draw() {
  ...
  let px = count;
  let py = anomaliesArray[0] * Math.pow(10, 5) + 350 *heightOfCanvasInitial/969*0.8;
  console.log(anomaliesArray[0]);
  console.log(anomaliesArray[300]);
  console.log(anomaliesArray[600]);
  console.log(anomaliesArray[900]);
  console.log(anomaliesArray[1200]);
  ...
  for (var i = 0; i < lim; i += 10){
    ...
    line(px, py-(50*heightOfCanvasInitial/969/0.8), x, y-(50*heightOfCanvasInitial/969/0.8));
    px = x;
    py = y;
    count += 2.925;
  }
